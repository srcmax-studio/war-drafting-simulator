...
import { Eye, Flag, LockKeyhole, RotateCcw, Skull, Swords, Timer, Undo2 } from 'lucide-vue-next';
import type { CardDefinition, DeploymentIntent, PlayerView } from '~/common/src/index';
    ...
    <div class="battle-layout">
      <section class="front-scroll" aria-label="三条战线">
        <article v-for="front in view.fronts" :key="front.definition.frontId" class="front-lane" :class="{ unrevealed: !front.revealed }">
          <div class="lane-cards enemy-line">
            ...
            <span v-if="enemyCards(front.definition.frontId).length === 0" class="slot-mark">0/4</span>
          </div>

          <button type="button" class="front-brief" :disabled="!front.revealed" @click="chooseFront(front.definition.frontId)">
            ...
            <template v-else><LockKeyhole :size="22" /><strong>未揭示战线</strong><p>第 {{ view.fronts.indexOf(front) + 1 }} 回合公开</p></template>
          </button>

          <div class="lane-cards own-line">
            ...
            <button v-if="ownCards(front.definition.frontId).length + planned(front.definition.frontId).length < 4" type="button" class="slot-mark deploy-slot" @click="chooseFront(front.definition.frontId)">+</button>
          </div>
        </article>
      </section>
      ...
      <aside class="event-panel">
        ...
        <ol><li v-for="event in view.events.slice(-16).reverse()" :key="event.sequence"><time>R{{ event.turn }}</time><span>{{ $t(`events.${event.type}`) }}</span></li></ol>
      </aside>
    </div>
