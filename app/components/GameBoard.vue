...
import { TRIGGER_LABELS, deploymentFailureLabel, eventLabel, formatCardStatus } from '~/utils/ability-text';
...
const eventDetail = (event: GameEvent) => {
  if (event.type === 'ability_started') return TRIGGER_LABELS[event.payload.trigger as keyof typeof TRIGGER_LABELS] ?? '特殊时机';
  if (event.type === 'ability_resolved') return `${event.payload.targets ?? 0} 目标 · ${event.payload.changed ?? 0} 变化`;
  if (event.type === 'deployment_fizzled') return deploymentFailureLabel(event.payload.reason);
  return '';
};
</script>
  ...
  <div class="battle-screen">
    <header class="battle-status">
      ...
      <div class="turn-cluster">
        ...
        <span v-if="remainingSeconds !== null"><Timer :size="16" /> {{ remainingSeconds }} 秒</span>
      </div>
    ...
    <div class="battle-layout">
        ...
        <article v-for="front in view.fronts" :key="front.definition.frontId" class="front-lane" :class="{ unrevealed: !front.revealed }">
          <div class="lane-cards enemy-line" :style="{ gridTemplateColumns: `repeat(${Math.min(6, capacity(front, opponent.playerId))}, minmax(0, 1fr))` }">
            <button v-for="instance in enemyCards(front.definition.frontId)" :key="instance.instanceId" type="button" class="board-card" @click="instanceCard(instance) && (detailCard = instanceCard(instance))">
              ...
              <template v-else><div class="card-back-mini">万世</div><small>伏兵</small></template>
            </button>
          ...
          <div class="lane-cards own-line" :style="{ gridTemplateColumns: `repeat(${Math.min(6, capacity(front, playerId))}, minmax(0, 1fr))` }">
            <button v-for="instance in ownCards(front.definition.frontId)" :key="instance.instanceId" type="button" class="board-card" @click="instanceCard(instance) && (detailCard = instanceCard(instance))">
              <template v-if="instanceCard(instance)">
                ...
                <i v-if="Object.keys(instance.markers ?? {}).length || (instance.statuses?.length ?? 0)" class="instance-state">{{ Object.entries(instance.markers ?? {}).map(([name,count]) => `${name}${count}`).join(' ') }} {{ instance.statuses?.map(formatCardStatus).join(' ') }}</i>
              </template>
            </button>
      ...
      <aside class="event-panel">
        ...
        <ol><li v-for="event in view.events.slice(-24).reverse()" :key="event.sequence" :class="{ 'ability-event': event.type.startsWith('ability_') }"><time>第{{ event.turn }}回合</time><span>{{ describeEvent(event) }}<small v-if="eventDetail(event)">{{ eventDetail(event) }}</small></span></li></ol>
      </aside>
    </div>

    <footer class="command-deck">
      <div class="player-strip">
        <div><strong>{{ player.name }}</strong><span>{{ $t('game.deck') }} {{ player.deckCount }} · <Skull :size="14" /> {{ player.graveyardCount }}</span></div>
