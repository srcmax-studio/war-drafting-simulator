...
<template>
  <div class="page deck-page">
    ...
    <div v-if="newDialog" class="modal-backdrop" role="presentation" @click.self="newDialog = false">
      <section class="dialog compact-dialog" role="dialog" aria-modal="true" aria-label="新建牌组">
        <header class="dialog-header"><strong>新建自定义牌组</strong><button class="icon-button" type="button" aria-label="关闭" @click="newDialog = false"><X :size="19" /></button></header>
    ...
    <div v-if="importDialog" class="modal-backdrop" role="presentation" @click.self="importDialog = false">
    ...
    <div v-if="importPreview || sharePreview" class="modal-backdrop" role="presentation" @click.self="closeImportPreview">
      <section class="dialog compact-dialog" role="dialog" aria-modal="true" aria-label="牌组预览">
        ...
        <div v-else class="dialog-content"><p class="danger">{{ importError || '分享链接无法解码。' }}</p><div class="dialog-actions"><button class="button" type="button" @click="closeImportPreview">关闭</button></div></div>
      </section>
    </div>

    <div v-if="pendingDelete" class="modal-backdrop" role="presentation" @click.self="pendingDelete = false">
      <section class="dialog confirm-dialog" role="alertdialog" aria-modal="true" aria-label="删除牌组">
        <div class="dialog-content"><h2>删除“{{ selectedDeck?.name }}”？</h2><p>此操作只删除当前自定义牌组，最近备份仍可用于恢复。</p><div class="dialog-actions"><button class="button" type="button" @click="pendingDelete = false">取消</button><button class="button danger-button" type="button" @click="confirmDelete"><Trash2 :size="17" /> 删除</button></div></div>
      </section>
    </div>
  </div>
</template>
