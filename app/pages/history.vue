...
<template>
  <div class="page">
    <header class="page-heading"><div><span class="eyebrow">对局记录</span><h1>对局战史</h1><p>查看使用的牌组、最终结果和战况记录；本地演武可完整重放。</p></div><button class="button danger-button" type="button" :disabled="history.length === 0" @click="clear"><Trash2 :size="17" /> 清空</button></header>
    <div v-if="history.length" class="history-layout">
      ...
      <section v-if="selected" class="history-detail">
        <div class="history-head"><div><span class="eyebrow">{{ selected.mode === 'practice' ? '本地演武' : '在线对战' }}</span><h2>{{ selected.deckName }}</h2><p>{{ format(selected.timestamp) }}</p></div><button class="icon-button" type="button" aria-label="删除" @click="remove(selected.gameId); selectedId = ''"><Trash2 :size="17" /></button></div>
        <div class="replay-check" :class="{ valid: replayValid === true, public: replayValid === null }"><CheckCircle2 v-if="replayValid === true" :size="20" /><XCircle v-else-if="replayValid === false" :size="20" /><Radio v-else :size="20" /><span>{{ replayValid === true ? '事件重放与最终状态完全一致' : replayValid === false ? '事件重放校验失败' : '在线战况已保存；出于公平，仅记录对局公开信息' }}</span></div>
        ...
        <h3>事件记录</h3><ol class="history-events"><li v-for="event in events.slice(-30).reverse()" :key="event.sequence"><time>第{{ event.turn }}回合</time><span>{{ describeEvent(event) }}</span></li></ol>
      </section>
